...
final DateTime STARTDATE = DateTime(2021, 10, 6);
final DateTime ENDDATE = DateTime(2022, 4, 1);
...
void main(List<String> args) async {
  ...
  addToFile('planning/metrics.log',
...
class Section {
  ...
  double size;
  double done;
  Section(this.group, this.size, this.done);
} // of section
...
class Task {
  ...
  Task.fromLine(List<String> thisLine) {
    ...
    size = thisLine[3].toUpperCase();
    progress = thisLine[4].toUpperCase();
  ...
  static void saveCarPark(List<Task> taskList) {
    ...
    for (var task in taskList) {
      if (task.group.isNotEmpty || task.sizeInUnits > 0) {
        ...
        for (var sectionIx = 0; sectionIx < sections.length; sectionIx++) {
          if (sections[sectionIx].group == task.group) {
            foundSectionIx = sectionIx;
          }
        }
        if (foundSectionIx == -1) {
          sections.add(Section(task.group, 0.0, 0.0));
          foundSectionIx = sections.length - 1; // point to last item
        }
        sections[foundSectionIx].size += task.sizeInUnits;
        sections[foundSectionIx].done += task.sizeInUnits - task.outstandingInUnits;
    ...
    for (var section in sections) {
      logFile.writeln('${pad(section.group, 20)}' +
          done.substring(0, (section.done).round()) +
          todo.substring(0, (section.size - section.done).round()) +
          pad('', 60 - section.size.round()) +
          displayPair(section.done, section.size));
    }
    logFile.writeln();
